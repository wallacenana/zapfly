...
function buildStoreProfileData(payload = {}) {
  ...
  const placeId = String(payload.businessPlaceId ?? (payload.businessAddress !== undefined ? '' : legacyLocation.placeId) ?? '').trim();
  const inputLat = toNumberOrNull(payload.businessLat);
  const inputLng = toNumberOrNull(payload.businessLng);
  const hasAddressPayload = payload.businessAddress !== undefined;
  const lat = inputLat !== null && inputLat !== 0
    ? inputLat
    : (hasAddressPayload ? null : legacyLocation.lat);
  const lng = inputLng !== null && inputLng !== 0
    ? inputLng
    : (hasAddressPayload ? null : legacyLocation.lng);
  ...
  return cleanDefined({
    ...
    businessLat: lat,
    businessLng: lng,
    businessMapsUrl: String(payload.businessMapsUrl || (hasAddressPayload ? '' : legacyLocation.mapsUrl) || '').trim() || undefined,
...
function mergeStoreProfile({ setting = null, storeProfile = null, user = null } = {}) {
  ...
  const profileHasAddress = Boolean(String(profile.businessAddress || '').trim());
  const mergedLocation = {
    address: profile.businessAddress || fallback.businessAddress || '',
    placeId: profile.businessPlaceId || (profileHasAddress ? '' : fallback.businessPlaceId) || '',
    lat: profile.businessLat ?? (profileHasAddress ? null : fallback.businessLat) ?? null,
    lng: profile.businessLng ?? (profileHasAddress ? null : fallback.businessLng) ?? null,
    mapsUrl: profile.businessMapsUrl || (profileHasAddress ? '' : fallback.businessMapsUrl) || ''
  };
