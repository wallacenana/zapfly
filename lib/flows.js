        ...
        if (state && state.currentNodeId) {
            ...
            if (flow && flow.status === 'Ativo') {
                ...
                if (nextEdges.length > 0) {
                    ...
                    await runFlowNode(sock, instanceId, jid, flow, nextEdges[0].target, null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName, images, text, userId);
                    return true;
                } else {
                    // Fim do fluxo
    ...
    try {
        ...
        if (outgoingEdges.length > 0) {
            for (const edge of outgoingEdges) {
                await runFlowNode(sock, instanceId, jid, flow, edge.target, edge.sourceHandle, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName, images, currentText, userId);
            }
        } else {
            await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
            if (io) io.emit('chat_update', { instanceId, jid, inFlow: false });
