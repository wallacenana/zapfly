...
const { getStoreStatus, hasAvailableProductStock, getDeliveryCatalog, getOrderCatalog, getRestaurantGreeting, shouldSendRestaurantGreeting } = require('./utils');
...
async function handleFlows(sock, instanceId, jid, text, msg, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName = "", images = [], userId) {
    try {
        console.log(`[Flow][ENTRY] instance=${instanceId} user=${userId} jid=${jid} textLength=${String(text || '').length}`);
        ...
        if (state && state.currentNodeId) {
            console.log(`[Flow][STATE] instance=${instanceId} jid=${jid} flow=${state.flowId} node=${state.currentNodeId} resumeAt=${state.resumeAt?.toISOString?.() || 'none'}`);
        ...
        for (const flow of flows) {
            try {
                ...
                if (match) {
                    console.log(`[Flow][TRIGGER] instance=${instanceId} jid=${jid} flow=${flow.id} node=${triggerNode.id} config=${config.configuracao || 'unknown'}`);
...
async function runFlowNode(sock, instanceId, jid, flow, nodeId, sourceHandle = null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName = "", images = [], currentText = "", userId) {
    try {
        ...
        console.log(`[Flow][NODE] instance=${instanceId} user=${userId} jid=${jid} flow=${flow.id} node=${node.id} type=${node.type}`);
        ...
        switch (node.type) {
            ...
            case 'aiNode': {
                ...
                console.log(`[Flow][AI_STORE] instance=${instanceId} jid=${jid} status=${storeInfo.statusLoja} time=${storeInfo.horaAtual} day=${storeInfo.nomeDia} promptLength=${nodePrompt.length}`);
                ...
                console.log(`[Flow][AI_CONTEXT] instance=${instanceId} jid=${jid} historyCount=${history.length} messageCount=${messages.length} systemPromptLength=${finalSystemPrompt.length}`);
                ...
                const tools = [
                    ...
                    },
                    {
                        ...
                        function: {
                            ...
                            description: "OBRIGATÓRIO: Chame quando o cliente pedir encomendas, agendamentos, bolos ou itens para reservar. NUNCA liste produtos manualmente.",
                            parameters: { type: "object", properties: {} }
                        }
                    },
                    {
                ...
                console.log(`[Flow][AI_INTENT] instance=${instanceId} jid=${jid} status=${storeInfo.statusLoja} delivery=${asksForDeliveryCatalog} order=${asksForOrderCatalog} textLength=${normalizedRequest.length}`);

                if (storeInfo.statusLoja === 'FECHADA' && asksForDeliveryCatalog && !asksForOrderCatalog) {
                    console.log(`[Flow][BLOCKED] delivery catalog blocked because store is closed: instance=${instanceId} jid=${jid} time=${storeInfo.horaAtual}`);
                    if (await shouldSendRestaurantGreeting(instanceId, jid)) {
                        await sock.sendMessage(jid, { text: await getRestaurantGreeting(instanceId, userId) });
                    }
                    await sock.sendMessage(jid, { text: 'A loja está fechada no momento. A pronta-entrega funciona dentro do horário de atendimento.' });
                    ...
                    console.log(`[Flow][CATALOG_TOMORROW] instance=${instanceId} user=${userId} products=${tomorrowCatalog.count} chars=${tomorrowCatalog.text.length}`);
                    return true;
                }

                const toolChoice = asksForOrderCatalog
                    ? { type: 'function', function: { name: 'get_order_catalog' } }
                    : asksForDeliveryCatalog
                        ? { type: 'function', function: { name: 'get_delivery_catalog' } }
                        : 'auto';
                console.log(`[Flow][AI_ROUTE] instance=${instanceId} jid=${jid} tool=${typeof toolChoice === 'string' ? toolChoice : toolChoice.function.name} model=${modelToUse}`);
                ...
                console.log(`[Flow][AI_RESPONSE] instance=${instanceId} jid=${jid} toolCalls=${responseMessage.tool_calls?.length || 0} contentLength=${responseMessage.content?.length || 0}`);
                ...
                if (responseMessage.tool_calls) {
                    ...
                    for (const toolCall of responseMessage.tool_calls) {
                        ...
                        console.log(`[Flow][TOOL_START] instance=${instanceId} jid=${jid} tool=${functionName}`);
                        if (functionName === "get_delivery_catalog") {
                            ...
                            console.log(`[Flow][CATALOG_TOOL] instance=${instanceId} user=${userId} jid=${jid} status=${statusLoja}`);
                            if (statusLoja === 'FECHADA') {
                                console.log(`[Flow][CATALOG_BLOCKED] instance=${instanceId} jid=${jid} reason=store_closed`);
                                await sock.sendMessage(jid, { text: 'A loja está fechada no momento. A pronta-entrega funciona dentro do horário de atendimento.' });
                                return true;
                            }
                            ...
                            console.log(`[Flow][CATALOG_READY] instance=${instanceId} jid=${jid} products=${prods.length} chars=${catalogText.length}`);
                            ...
                            try {
                                ...
                                console.log(`[Flow][ORDER_CATALOG] instance=${instanceId} user=${userId} products=${catalog.count} chars=${catalogText.length}`);
                                ...
                                await new Promise(r => setTimeout(r, 1200));
                                await sock.sendPresenceUpdate('paused', jid).catch(() => { });
                                await sock.sendMessage(jid, { text: 'Qual destes mais te encantou? Posso te ajudar a escolher o tamanho ideal para sua festa! 😊✨' });
