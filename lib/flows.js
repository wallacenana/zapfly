...
const { getStoreStatus, hasAvailableProductStock, formatProduct, getDeliveryCatalog, getOrderCatalog, ensureRestaurantGreeting, getClosedDeliveryMessage, isSimpleGreeting } = require('./utils');
const { MODEL_MAP } = require('./ai');
const { isDeliveryOrderFollowUp, isCatalogRequest, isFinalOrderConfirmation, findSelectedProduct } = require('./utils');
 ...
 */
async function handleFlows(sock, instanceId, jid, text, msg, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName = "", images = [], userId) {
    try {
        ...
        if (state && state.currentNodeId) {
            ...
            if (flow && flow.status === 'Ativo') {
                ...
                if (nextEdges.length > 0) {
                    ...
                    const handled = await runFlowNode(sock, instanceId, jid, flow, nextEdges[0].target, null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName, images, text, userId);
                    if (handled === false) {
                        console.warn(`[Flow] Fluxo ${flow.id} falhou ao executar o próximo nó para ${jid}. Liberando a conversa.`);
                ...
                console.warn(`[Flow] Estado perdido/inativo para ${jid} na instancia ${instanceId}. Limpando estado antigo.`);
                await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
                if (io) io.emit('chat_update', { instanceId, jid, inFlow: false });
            }
        }
        ...
        for (const flow of flows) {
            try {
                ...
                if (match) {
                    ...
                    await runFlowNode(sock, instanceId, jid, flow, triggerNode.id, null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName, images, text, userId);
                    return true;
                }
            } catch (e) {
 ...
 */
async function runFlowNode(sock, instanceId, jid, flow, nodeId, sourceHandle = null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName = "", images = [], currentText = "", userId) {
    try {
        const flowData = JSON.parse(flow.data);
        const node = flowData.nodes.find(n => n.id === nodeId);
        if (!node) {
            console.warn(`[Flow] Nó ${nodeId} não encontrado no fluxo ${flow.id}. Limpando estado de ${jid}.`);
            await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
            if (io) io.emit('chat_update', { instanceId, jid, inFlow: false });
            return false;
        }
        ...
        switch (node.type) {
            ...
            case 'aiNode': {
                ...
                if (greeted && images.length === 0 && isSimpleGreeting(currentText)) {
                    return true;
                }
                const ai = await getOpenAI(userId);
                if (!ai) {
                    console.warn(`[Flow] Nó AI ${node.id} sem configuração OpenAI para ${jid} na instância ${instanceId}.`);
                    return false;
                }

                const nodePrompt = node.data?.prompt || node.data?.instruction || "";
                const storeInfo = await getStoreStatus(userId);
                const finalSystemPrompt = await buildLilyPrompt(instanceId, jid, nodePrompt, storeInfo, pushName, userId);
                await sock.sendPresenceUpdate('composing', jid).catch(() => { });
                ...
                const messages = [
                    ...
                    ...history.reverse().filter(m => m.fromMe || m !== history[history.length - 1] || m.text !== currentText).map(m => ({
                        role: m.fromMe ? 'assistant' : 'user',
                        content: m.text || (m.fromMe ? 'Mensagem da assistente' : 'Mensagem do Cliente')
                    })),
                ...
                const tools = [
                    {
                        ...
                        function: {
                            name: "create_order",
                            description: "Cria um pedido e gera o link de pagamento. Para encomenda (type order), use SOMENTE apos item, variacao, data e horario disponiveis, adicionais/opcoes cadastrados, resumo apresentado e confirmacao explicita. Para delivery (type delivery), e para hoje: nao exija data ou horario; colete item, variacao, adicionais/opcoes, endereco quando necessario, resumo e confirmacao. Nunca pergunte o nome: use o nome ja presente no contexto, se houver. Uma escolha de produto ou opcao nao e confirmacao final. Nao invente etapas de massa ou recheio. Colete uma etapa por mensagem. Importacoes administrativas seguem as instrucoes do administrador.",
                            parameters: {
                                type: "object",
                                properties: {
                                    product: { type: "string" },
                                    subItem: { type: "string", description: "Subvariacao/opcao escolhida dentro da variacao." },
                                    ...
                                    massa: { type: "string", description: "Tipo da massa do bolo, somente quando identificado explicitamente como massa" },
                                    recheio: { type: "string", description: "Sabor do recheio, somente quando identificado explicitamente como recheio" },
                                    ...
                                    notes: { type: "string", description: "Observações e respostas dos campos extras no formato 'Nome do campo: resposta'. Para imagem, use 'Nome do campo: X imagens recebidas' com a quantidade real recebida." }
                                },
                        ...
                        function: {
                            ...
                            parameters: {
                                ...
                                properties: {
                                    ...
                                    type: { type: "string", enum: ["order", "delivery"], description: "Tipo do pedido" }
                                },
                                required: ["date", "type"]
                            }
                        }
                ...
                const normalizedRequest = String(currentText || '').toLowerCase();
                const deliveryOrderFollowUp = isDeliveryOrderFollowUp(messages, currentText);
                const requestsCatalog = isCatalogRequest(currentText);
                const finalOrderConfirmed = isFinalOrderConfirmation(messages, currentText);
                const selectedProduct = requestsCatalog ? null : await findSelectedProduct(userId, currentText, deliveryOrderFollowUp, messages);
                ...
                const asksForOrderCatalog = /encomenda|bolo de festa|personalizado|agendar|agendamento/.test(normalizedRequest);
                const asksForDeliveryCatalog = !asksForOrderCatalog && /delivery|pronta entrega|o que tem|o que temos|temos hoje|tem hoje|tem pra hoje|para hoje|dispon[ií]vel|card[aá]pio|cat[aá]logo|opções|opcoes|opção|opcao|pre[cç]o/.test(normalizedRequest);

                const explicitOrderCatalogRequest = /encomenda|bolo de festa|personalizado/.test(normalizedRequest)
                    && /card[aá]pio|cat[aá]logo|lista|op[cç][õo]es|opcoes|o que tem|o que temos|quais/.test(normalizedRequest);
                const explicitDeliveryCatalogRequest = !explicitOrderCatalogRequest
                    && /delivery|pronta entrega|o que tem.*hoje|temos hoje|tem (?:pra |para )?hoje/.test(normalizedRequest);

                if (storeInfo.statusLoja === 'FECHADA' && explicitDeliveryCatalogRequest && !asksForOrderCatalog) {
                    await sock.sendMessage(jid, { text: getClosedDeliveryMessage(settings) });
                    return true;
                }

                const toolChoice = finalOrderConfirmed
                    ? { type: 'function', function: { name: 'create_order' } }
                    : (explicitOrderCatalogRequest || (deliveryOrderFollowUp && requestsCatalog))
                    ? { type: 'function', function: { name: 'get_order_catalog' } }
                    : explicitDeliveryCatalogRequest
                        ? { type: 'function', function: { name: 'get_delivery_catalog' } }
                        : 'auto';
                ...
                const completion = await ai.chat.completions.create({
                    ...
                    tool_choice: selectedProduct && ['get_delivery_catalog', 'get_order_catalog'].includes(toolChoice?.function?.name) ? 'auto' : toolChoice
                });
                ...
                if (responseMessage.tool_calls) {
                    ...
                    for (const toolCall of responseMessage.tool_calls) {
                        ...
                        console.log('[Flow][TOOL_CALL]', JSON.stringify({ instanceId, jid, functionName, args }));
                        if (functionName === "get_delivery_catalog") {
                            ...
                            const prods = await getCachedProducts(userId);
                            let deliveryStr = '';
                            prods.filter(p => p.active !== false
                                ...
                                .filter(p => {
                                    ...
                                    return hasAvailableProductStock(p, vars);
                                }).forEach(p => {
                                const vars = typeof p.variations === 'string' ? JSON.parse(p.variations || '[]') : (p.variations || []);
                            ...
                            if (!statusLoja.includes('FECHADA')) {
                                introMsg = 'Hoje teremos essas delícias:';
                            ...
                            try {
                                ...
                                // Balão 1: Intro (Vem da IA se ela gerou, ou padrão)
                                // Nota: No flows.js a IA geralmente já mandou a intro antes da tool call
                                await sock.sendMessage(jid, { text: deliveryOrderFollowUp ? 'Os itens do delivery que você pode encomendar são:' : 'Estas são nossas opções para encomenda:' });

                                // Balão 2: Catálogo
                                await sock.sendPresenceUpdate('composing', jid).catch(() => { });
                                await new Promise(r => setTimeout(r, Math.min(catalogText.length * 5, 3000)));
                                await sock.sendPresenceUpdate('paused', jid).catch(() => { });
                                await sock.sendMessage(jid, { text: catalogText });
                                ...
                                if (node.data?.activateAgent) {
                                    await prisma.chat.update({
                                        where: { jid_instanceId: { jid, instanceId } },
                                        data: { aiEnabled: true, aiManuallyPaused: false }
                                    });
                                    await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
                                    if (io) io.emit('chat_update', { instanceId, jid, aiEnabled: true, inFlow: false });
                                }
                            ...
                            try {
                                ...
                                console.error(`[Flow][CALCULATE_ORDER_TOTAL_ERROR] instance=${instanceId} jid=${jid} message=${error.message}`);
                            ...
                            try {
                                // BLOQUEIO DE SEGURANÇA: Evita duplicatas se o cliente já tem um pedido aberto (15 min)
                                ...
                                if (recentOrder) {
                                    const ref = recentOrder.id.slice(-5).toUpperCase();
                                    await sock.sendMessage(jid, { text: `⚠️ *Aviso:* Notei que você já tem um pedido em aberto (#${ref}). Vou atualizar ele com essas novas informações! ✨` });
                                    const res = await axios.patch(`http://127.0.0.1:3001/orders/${recentOrder.id}`, args, {
                                        headers: {
                                    ...
                                    if (res.data.paymentLink) {
                                        ...
                                        await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
                                        return { success: true, message: "Pedido atualizado e link enviado.", paymentLink: res.data.paymentLink };
                                    }
                                    return { success: true, message: "Pedido atualizado." };
                                }

                                const res = await axios.post('http://127.0.0.1:3001/orders', {
                                    ...args,
                                    ...
                                    clientName: args.clientName || pushName || undefined,
                                    clientJid: jid,
                                ...
                                orderError = err.response?.data?.error || err.message;
                                console.error(`[Flow][CREATE_ORDER_ERROR] instance=${instanceId} jid=${jid} status=${err.response?.status || 'n/a'} message=${err.response?.data?.error || err.message}`);
                                result = { success: false, error: orderError };
                            }
                        } else if (functionName === "update_order") {
                            try {
                                if (!args.orderId) {
                                    result = { success: false, error: "ID do pedido não fornecido pela IA." };
                                } else {
                                    const refCode = args.orderId.replace('#', '').trim().toUpperCase();

                                    const allOrders = await prisma.order.findMany({
                                        where: jid ? { clientJid: jid } : {},
                                        orderBy: { createdAt: 'desc' }
                                    });

                                    const targetOrder = allOrders.find(o => {
                                        const fullId = o.id.toUpperCase();
                                        return fullId.endsWith(refCode) || refCode.endsWith(fullId.slice(-5).toUpperCase());
                                    });

                                    if (!targetOrder) {
                                        result = { success: false, error: "Pedido não encontrado." };
                                    } else {
                                        const axios = require('axios');
                                        const res = await axios.patch(`http://127.0.0.1:3001/orders/${targetOrder.id}`, args, {
                                            headers: {
                                                'x-internal-token': process.env.INTERNAL_TOKEN || 'menzzu-internal-bypass-key',
                                                'x-user-id': userId
                                            }
                                        });
                                        result = { success: true, message: "Pedido atualizado." };
                                        ...
                                        if (res.data.paymentLink) {
                                            try {
                                                ...
                                                await sock.sendMessage(jid, { text: 'O pedido será confirmado após o pagamento.' });
                                                await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
                                                return; // Mata o fluxo aqui para não mandar balão extra
                                            } catch (sendErr) { }
                                        }
                                    }
                                }
                    ...
                    if (orderError) {
                        await sock.sendMessage(jid, { text: `Não consegui registrar o pedido. Motivo: ${orderError}` });
                        return true;
                    }
            ...
            case 'notifyNode': {
                ...
                if (adminPhone) {
                    ...
                    const chatInfo = await prisma.chat.findUnique({ where: { jid_instanceId: { jid, instanceId } } });
        ...
        if (outgoingEdges.length > 0) {
            for (const edge of outgoingEdges) {
                const handled = await runFlowNode(sock, instanceId, jid, flow, edge.target, edge.sourceHandle, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName, images, currentText, userId);
                if (handled === false) return false;
            }
            return true;
        } else {
            console.warn(`[Flow] Nó ${node.id} terminou sem próximas conexões no fluxo ${flow.id}. Encerrando estado.`);
            await prisma.flowState.deleteMany({ where: { instanceId, jid } }).catch(() => { });
            if (io) io.emit('chat_update', { instanceId, jid, inFlow: false });
            return true;
        }
