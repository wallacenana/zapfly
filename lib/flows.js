 ...
 */
async function handleFlows(sock, instanceId, jid, text, msg, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName = "") {
    try {
        ...
        if (state && state.currentNodeId) {
            ...
            if (flow && flow.status === 'Ativo') {
                ...
                if (nextEdges.length > 0) {
                    ...
                    await runFlowNode(sock, instanceId, jid, flow, nextEdges[0].target, null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName);
                    return true;
                } else {
                    // Fim do fluxo
        ...
        for (const flow of flows) {
            try {
                ...
                if (match) {
                    ...
                    await runFlowNode(sock, instanceId, jid, flow, triggerNode.id, null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName);
                    return true;
                }
            } catch (e) {
 ...
 */
async function runFlowNode(sock, instanceId, jid, flow, nodeId, sourceHandle = null, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName = "") {
    try {
        const flowData = JSON.parse(flow.data);
                ...
                const tools = [
                    {
                        ...
                        function: {
                            name: "create_order",
                            description: "Cria um novo pedido. REGRA: NUNCA duplique pedidos. Se o cliente estiver corrigindo ou tentando de novo, use 'update_order'.",
                            parameters: {
                                type: "object",
                                properties: {
                                    product: { type: "string" },
                                    ...
                                    type: { type: "string", enum: ["order", "delivery"], description: "OBRIGATÓRIO: 'delivery' para pronta entrega/agora. 'order' para encomendas/agendamentos futuros." },
                                    notes: { type: "string" }
                                },
        ...
        if (outgoingEdges.length > 0) {
            for (const edge of outgoingEdges) {
                await runFlowNode(sock, instanceId, jid, flow, edge.target, edge.sourceHandle, buildLilyPrompt, getOpenAI, executeChamarGerente, settings, pushName);
            }
        } else {
