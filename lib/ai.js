...
function getEnabledFulfillmentMethods(settings, orderType = 'delivery') {
    ...
    const methodsKey = orderType === 'order' ? 'orderFulfillmentMethods' : 'fulfillmentMethods';
    const configuredMethods = configured?.[methodsKey];
    const hasExplicitFulfillmentMethods = configuredMethods
        && typeof configuredMethods === 'object';
    if (hasExplicitFulfillmentMethods) {
        Object.keys(methods).forEach(method => {
            methods[method] = configuredMethods[method] === true;
        });
    }

    if (!hasExplicitFulfillmentMethods) {
        if (orderType === 'order') {
            return { delivery: false, pickup: true, local: false };
        }
...
function formatFulfillmentMethods(settings, orderType = 'delivery') {
    const enabled = getEnabledFulfillmentMethods(settings, orderType);
    const labels = {
...
Pronta-entrega: ${formatFulfillmentMethods(settings, 'delivery')}.
Encomendas: ${formatFulfillmentMethods(settings, 'order')}.
...
- Com todos os dados completos, apresente um unico resumo e pergunte se pode confirmar. Para encomenda, inclua item, variacao, opcoes/adicionais, informacoes extras de texto/lista, data, hora e valor total. Para delivery, inclua item, variacao, opcoes/adicionais, informacoes extras de texto/lista, forma de recebimento/endereco quando houver e valor total, sem data ou hora. Campos de imagem nao entram como texto nem com contagem no resumo: os anexos sao exibidos apenas na galeria do pedido. Aguarde a resposta. Somente apos uma confirmacao explicita desse resumo use create_order e envie o link retornado pela ferramenta. Se houver correcao, atualize o resumo e solicite nova confirmacao antes de criar o pedido.
