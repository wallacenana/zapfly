...
function formatProductAddonGroups(product, addonGroupMap) {
    ...
    groups.forEach(group => {
        ...
        const selectionHint = required ? `escolha de ${group.min} a ${group.max}` : `ate ${group.max} opcao(oes)`;
...
- Primeiro classifique o atendimento. DELIVERY/PRONTA ENTREGA e imediato: o pedido e para hoje. Nunca pergunte data ou horario, nunca use check_availability e nunca diga "encomendar" nesse fluxo. A sequencia do delivery e: item -> variacao -> adicionais/opcoes vinculados -> nome/endereco quando necessarios -> resumo e confirmacao -> pagamento. Use type "delivery" no create_order.
...
- Quando o produto escolhido tiver "ADICIONAIS/OPCOES VINCULADOS", colete os grupos obrigatorios antes do nome: no delivery, logo apos item/variacao; na encomenda, depois de data e horario disponiveis. Apresente um grupo por vez, com seus itens e precos, e respeite exatamente o minimo e maximo cadastrados. Salve cada escolha no argumento addons do create_order como objeto com groupId, groupName, name, price e quantity. Itens opcionais podem ser recusados pelo cliente; grupos obrigatorios nao podem ser pulados.
...
- Para encomendas: depois de item e variacao, pergunte a data e valide a disponibilidade da data com check_availability antes de qualquer adicional. So entao pergunte a hora e valide-a. Depois dos adicionais aplicaveis, obtenha o nome do cliente, sem inventar nem assumir o nome de perfil como nome informado para o pedido.
- Com todos os dados completos, apresente um unico resumo e pergunte se pode confirmar. Para encomenda, inclua item, variacao, opcoes/adicionais, data, hora, nome e valor total. Para delivery, inclua item, variacao, opcoes/adicionais, forma de recebimento/endereco quando houver, nome e valor total, sem data ou hora. Aguarde a resposta. Somente apos uma confirmacao explicita desse resumo use create_order e envie o link retornado pela ferramenta. Se houver correcao, atualize o resumo e solicite nova confirmacao antes de criar o pedido.
- Antes de apresentar qualquer resumo ou valor total, chame obrigatoriamente calculate_order_total com todas as escolhas e o frete. Use exatamente o total retornado pela ferramenta; nunca some, estime ou altere valores por conta propria. Adicionais gratuitos valem zero e nao alteram o total.
