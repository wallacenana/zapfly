...
function formatProductAddonGroups(product, addonGroupMap) {
    ...
    groups.forEach(group => {
        ...
        parseJsonArray(group.items).forEach(item => {
            if (!item?.hidden) lines.push(`      • ${item.name || 'Opcao'}: R$ ${Number(item.price || 0).toFixed(2)}`);
        });
    });
    return `\n${lines.join('\n')}`;
}
...
- Para encomendas, siga esta ordem: item -> variacao -> adicionais/opcoes cadastrados -> data -> hora -> nome do cliente -> resumo e confirmacao -> criacao do pedido e envio do link de pagamento.
- Quando o produto escolhido tiver "ADICIONAIS/OPCOES VINCULADOS", conclua todos os grupos obrigatorios antes da data. Apresente um grupo por vez, com seus itens e precos, e respeite exatamente o minimo e maximo cadastrados. Salve cada escolha no argumento addons do create_order como objeto com groupId, groupName, name, price e quantity. Itens opcionais podem ser recusados pelo cliente; grupos obrigatorios nao podem ser pulados.
...
- Depois da ultima opcao cadastrada, pergunte a data, depois a hora em outra mensagem. Valide data/hora com check_availability antes de seguir. Depois obtenha o nome do cliente, sem inventar nem assumir o nome de perfil como nome informado para o pedido.
...
- Exiba o preco completo de cada opcao conforme o catalogo, nunca a diferenca para a menor opcao e nunca com sinal de +. Ignore formatos antigos de acrescimo no historico.
