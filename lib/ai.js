        ...
        if (msg.tool_calls) {
            ...
            for (const call of msg.tool_calls) {
                ...
                if (call.function.name === "create_order") {
                    ...
                    const feeRes = await calculateFee(String(args.address || '').trim(), userId);
                    if (feeRes.error) throw new Error(feeRes.error);
                    ...
                    if (args.assetId) {
                        ...
                        await sendStatusMessage(sock, { image: await getStatusImage(asset.url), caption });
                    } else {
