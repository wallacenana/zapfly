...
`;
        ...
        const requestsStatus = /\b(status|story|stories)\b/i.test(String(text || ''));
        const response = await ai.chat.completions.create({
            model: "gpt-4o",
            messages: [{ role: "system", content: `${systemPrompt}\n${productRegistrationRule}` }, { role: "user", content: userMessageContent }],
            tools: [
            ...
            for (const call of msg.tool_calls) {
                ...
                if (call.function.name === "create_order") {
                    ...
                    if (args.assetId) {
                        ...
                        await sock.sendMessage('status@broadcast', { image: { url: asset.url }, caption }, getStatusSendOptions(sock));
                    } else {
                        if (!caption) throw new Error('Informe o texto do status.');
                        await sock.sendMessage('status@broadcast', { text: caption }, getStatusSendOptions(sock));
                    }
