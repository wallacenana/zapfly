...
async function calculateFee(clientAddress, userId) {
    ...
    try {
        ...
        // O limite absoluto de entrega vem da configuração (ou 15km por padrão)
        const hardLimit = settings.maxDeliveryKm ? parseFloat(settings.maxDeliveryKm) : 15;

        // Verifica limite máximo de entrega absoluto
        if (distanceKm > hardLimit) {
            return { error: `Fora do raio de entrega permitido de ${hardLimit.toFixed(1)}km (distância atual: ${distanceKm.toFixed(1)}km).`, distance: distanceKm.toFixed(1) };
        }
