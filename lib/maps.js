...
async function resolveStoreOrigin(settings, apiKey) {
    const configuredOrigin = getStoreOrigin(settings);
    if (!configuredOrigin || /^-?\d+(?:\.\d+)?,\s*-?\d+(?:\.\d+)?$/.test(configuredOrigin)) {
    ...
    try {
        ...
        const postalCode = configuredOrigin.match(/\b\d{5}-?\d{3}\b/)?.[0]?.replace('-', '');
        const selected = postalCode
            ? results.find(result => result.address_components?.some(component =>
                component.types?.includes('postal_code') && String(component.long_name || '').replace('-', '') === postalCode
            ))
            : results[0];
        ...
        console.error('[Maps Error] Falha ao localizar a origem:', error.message);
    }
...
async function calculateFee(clientAddress, userId, clientCoordinates = null) {
    ...
    try {
        const origin = await resolveStoreOrigin(settings, apiKey);
