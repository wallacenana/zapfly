...
async function calculateFee(clientAddress, userId) {
    ...
    try {
        ...
        // Com faixas configuradas, o modo híbrido continua sendo tabelado.
        // A estimativa automática só deve existir quando a loja escolheu
        // explicitamente "automatico" ou não cadastrou nenhuma faixa.
        if (mode === 'manual' || rules.length > 0) {
            const maxRule = rules.length > 0 ? rules[rules.length - 1].maxKm : 0;
            return { error: `Fora das faixas fixas de entrega. O limite cadastrado é ${maxRule}km.`, distance: distanceKm.toFixed(1) };
        }
