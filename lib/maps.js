...
async function calculateFee(clientAddress, userId) {
    ...
    try {
        ...
        const maxKmRule = rules.length > 0 ? rules[rules.length - 1].maxKm : (settings.maxDeliveryKm || 15);

        // Verifica limite máximo de entrega dinâmico (raio maior cadastrado no DB)
        if (distanceKm > maxKmRule) {
            return { error: `Fora do raio de entrega permitido de ${maxKmRule.toFixed(1)}km (distância atual: ${distanceKm.toFixed(1)}km).`, distance: distanceKm.toFixed(1) };
        }
