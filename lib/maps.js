...
async function calculateFee(clientAddress, userId) {
    ...
    try {
        ...
        // Regras cadastradas pela loja sempre vencem a estimativa por quilometragem.
        // Isso evita que o modo automatico ignore um frete fixo configurado.
        const matchingRule = rules.find(r => distanceKm <= Number(r.maxKm));

        if (matchingRule) {
            const fee = Number(matchingRule.fee);
            console.info('[Maps Fee] Regra fixa aplicada', {
                userId,
                mode,
                distanceKm: Number(distanceKm.toFixed(2)),
                maxKm: Number(matchingRule.maxKm),
                fee
            });
