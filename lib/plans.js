...
const getEntitledPlan = async (prisma, userId) => {
  ...
  if (['admin', 'superadmin'].includes(String(user?.role || '').trim().toLowerCase())) {
    return getPlan('unlimited');
  }
...
const checkEntitlement = async (prisma, userId, resource, currentCount) => {
  ...
  if (limit !== null && limit !== undefined && currentCount >= limit) {
    const resourceLabel = resource === 'productLimit' ? 'produtos' : resource === 'flowLimit' ? 'automações' : 'conexões WhatsApp';
    const error = new Error(`O plano ${plan.name} permite até ${limit} ${resourceLabel}.`);
