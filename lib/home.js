...
function buildStoreSummary(store, scheduleState = {}, reviewSummary = {}, orderSummary = {}) {
  ...
  const productCategories = [...new Set(visibleProducts
    .map((product) => String(product.categoryName || product.category || '').trim())
  ...
  const rawFulfillment = settings.dailyDeliveryItems;
  if (rawFulfillment) {
    try {
      const parsed = typeof rawFulfillment === 'string' ? JSON.parse(rawFulfillment) : rawFulfillment;
      if (parsed?.fulfillmentMethods && typeof parsed.fulfillmentMethods === 'object') {
        Object.keys(fulfillmentMethods).forEach((method) => {
          fulfillmentMethods[method] = parsed.fulfillmentMethods[method] !== false;
        });
      }
    } catch (error) {
      // Keep the delivery mode fallback below.
    }
  }
  const deliveryMode = String(settings.deliveryMode || 'hibrido').toLowerCase();
  if (!Object.values(fulfillmentMethods).some(Boolean)) {
    fulfillmentMethods.delivery = deliveryMode.includes('hibr') || deliveryMode.includes('deliver') || deliveryMode === 'entrega';
    fulfillmentMethods.pickup = deliveryMode.includes('hibr') || deliveryMode.includes('pickup') || deliveryMode.includes('retir');
    fulfillmentMethods.local = deliveryMode.includes('local') || deliveryMode.includes('consum');
  }

  const acceptOrders = isEnabled(settings.acceptOrders);
  const isOpenNow = acceptOrders && scheduleState.isOpenNow !== false;
  const scheduleLabel = !acceptOrders
    ? 'Fechado agora'
    : (scheduleState.statusLabel || 'Aberto');
  const scheduleClass = isOpenNow ? 'open' : 'closed';
  ...
  return {
    ...
    freeDeliveryEnabled,
    freeDeliveryKm,
    prepTime: String(settings.prepTime || '').trim(),
    deliveryMode,
    fulfillmentMethods,
    orderCount,
    ratingVisible,
    ...
    hasOperatingHours: Boolean(scheduleState.hasSchedule),
    hasDeliveryRadius: Number.isFinite(maxDeliveryKm) && maxDeliveryKm > 0,
    marketplaceEligible: Boolean(settings.logoUrl)
      && Boolean(scheduleState.hasSchedule)
      && Number.isFinite(maxDeliveryKm)
      && maxDeliveryKm > 0
...
async function loadDirectoryStoresRaw() {
  const baseColumns = `
    ...
    COALESCE(sp.deliveryMode, s.deliveryMode, 'hibrido') AS deliveryMode,
    s.dailyDeliveryItems AS dailyDeliveryItems,
...
async function buildHomeDirectoryData({ search = '', category = '', location = '', locationLat = null, locationLng = null, limit = DEFAULT_LIMIT } = {}) {
  ...
  const stores = rawStores.map((row) => ({
    ...
    settings: {
      ...
      dailyDeliveryItems: row.dailyDeliveryItems
    },
  ...
  const summaries = stores
    .map((store) => {
      ...
      if (Number.isFinite(queryLat) && Number.isFinite(queryLng)
        && Number.isFinite(summary.location?.lat) && Number.isFinite(summary.location?.lng)) {
        summary.distanceKm = haversineKm(queryLat, queryLng, summary.location.lat, summary.location.lng);
      }
      return summary;
    })
    .filter((store) => {
      if (store.active === false) return false;
      // Somente lojas configuradas para o marketplace aparecem aqui. Pedidos
      // pausados apenas deixam o card fechado, sem remover a loja da vitrine.
      if (!store.marketplaceEligible) return false;
...
function renderHeroRestaurants(restaurants = []) {
  ...
  return restaurants.slice(0, 4).map((store) => {
    ...
    const statusLabel = store.scheduleLabel || (isOpenNow ? 'Aberto' : 'Fechado agora');
    const statusClass = store.scheduleClass || (isOpenNow ? 'open' : 'closed');
...
function renderRestaurantCards(restaurants = []) {
  ...
  return restaurants.map((store) => {
    ...
    const ratingText = store.ratingVisible && store.ratingLabel
      ...
      : '';
    return `
      <article class="restaurant-card ${isOpenNow ? '' : 'is-closed'}">
        <a href="/${escapeAttr(store.slug)}" class="restaurant-card-link ${isOpenNow ? '' : 'is-closed'}">
          ...
          <div class="restaurant-card-content">
            <div class="restaurant-card-head">
              ...
              <span class="restaurant-status ${statusClass}">${escapeHtml(statusLabel)}</span>
            </div>
