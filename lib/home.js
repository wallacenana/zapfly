...
async function buildHomeDirectoryData({ search = '', category = '', location = '', limit = DEFAULT_LIMIT } = {}) {
  ...
  const summaries = stores
    ...
    .filter((store) => {
      ...
      if (locationQuery && !store.searchBlob.includes(locationQuery)) return false;
