...
const getProductId = (planKey, cycle) => {
  return String(process.env[`ABACATEPAY_${String(planKey).toUpperCase()}_${String(cycle).toUpperCase()}_PRODUCT_ID`] || '').trim();
};
...
const createSubscriptionCheckout = ({ planKey, cycle, externalId, metadata, customerId }) => {
  const productId = getProductId(planKey, cycle);
  if (!productId) {
