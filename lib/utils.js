...
async function getStoreStatus(userId) {
    ...
    console.log(`[StoreStatus] user=${userId} now=${hoje.toISOString()} day=${diaSemana} time=${horaAtual} slots=${JSON.stringify(slots.map(s => ({ start: s.startTime, end: s.endTime })))} status=${statusLoja} beforeOpening=${isBeforeOpening}`);
...
async function getRestaurantGreeting(instanceId, userId) {
    ...
    const category = String(settings?.businessCategory || '').toLowerCase();
    const feminine = /cake|doce|bolo|confeitaria|pizzaria|padaria|lanchonete|cafeteria|loja|empresa/.test(`${restaurantName} ${category}`.toLowerCase());
    ...
    return `Olá, eu sou ${assistantName}, ${article} agente virtual d${article} ${restaurantName}. Estou aqui para tirar suas dúvidas ou fazer seu pedido. Se quiser agilizar, acesse nosso cardápio digital: ${menuUrl}`;
}

async function shouldSendRestaurantGreeting(instanceId, jid) {
    const lastGreeting = await prisma.message.findFirst({
        where: {
            instanceId,
            jid,
            fromMe: true,
            OR: [
                { text: { startsWith: 'Olá, eu sou' } },
                { text: { startsWith: 'Ola, eu sou' } }
            ]
        },
    ...
    return Date.now() - lastGreeting.timestamp.getTime() >= 24 * 60 * 60 * 1000;
}
