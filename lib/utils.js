...
async function getOrderCatalog(userId, { deliveryOnly = false } = {}) {
    const { getCachedProducts, getSettings } = require('./cache');
    const [products, settings] = await Promise.all([getCachedProducts(userId), getSettings(userId)]);
    let menuOptions = null;
    try {
        menuOptions = typeof settings?.dailyDeliveryItems === 'string'
            ? JSON.parse(settings.dailyDeliveryItems)
            : settings?.dailyDeliveryItems;
    } catch (_) {
        menuOptions = null;
    }
    const includeDeliveryItems = menuOptions?.includeDeliveryItemsInOrders === true;
    const orderable = products.filter(product => {
        if (product.active === false) return false;
        const isDeliveryProduct = product.type === 'delivery' || product.type === 'combo_delivery';
        if (deliveryOnly) return includeDeliveryItems && isDeliveryProduct;
        return (includeDeliveryItems && isDeliveryProduct)
            || product.type === 'encomenda'
            || (String(product.type || '').startsWith('combo_') && !isDeliveryProduct);
    });
