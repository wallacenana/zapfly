...
try {
    ...
    $fallbackToWP = function () {
        $wp_index = defined('ABSPATH') ? ABSPATH . 'index.php' : __DIR__ . '/../../../../index.php';
...
?>
    ...
    // O cache de HTML fica sob responsabilidade do cache do WordPress.
    $bypassCache = true;
    ...
    $legacyLocation = json_decode((string) ($store['legacyBusinessLocation'] ?? ''), true);
    ...
    $storeLat = is_numeric($store['businessLat'] ?? null)
        ? (float) $store['businessLat']
        : (is_numeric($legacyLocation['lat'] ?? null) ? (float) $legacyLocation['lat'] : null);
    $storeLng = is_numeric($store['businessLng'] ?? null)
        ? (float) $store['businessLng']
        : (is_numeric($legacyLocation['lng'] ?? null) ? (float) $legacyLocation['lng'] : null);
    ...
    <head>
        ...
        <link rel="stylesheet" href="<?php echo esc_url(MENZZU_MARKETPLACE_URL . 'public-menu/style.css?v=3.97'); ?>">
        <style>
            :root {
                --primary-color:
    ...
    <body class="<?php echo $isDarkTheme ? 'theme-dark' : 'theme-light'; ?>">
        ...
        <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11" defer></script>
                <script type="text/javascript" src="<?php echo esc_url(MENZZU_MARKETPLACE_URL . 'public-menu/script.js?v=2.25'); ?>" defer></script>
