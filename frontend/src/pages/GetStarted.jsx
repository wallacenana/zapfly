  ...
  const renderControl = () => {
    if (current.type === 'delivery-options') {
      ...
      return <div style={{ display: 'grid', gap: '16px' }}>
        ...
        <div><strong style={{ display: 'block', marginBottom: '9px', fontSize: '13px' }}>Formas de retirada para Delivery</strong><div style={{ display: 'grid', gap: '9px' }}>{[['delivery', 'Entrega'], ['pickup', 'Retirada na loja'], ['local', 'Consumo no local']].map(([key, label]) => <button type="button" key={key} onClick={() => toggle('fulfillmentMethods', key)} style={{ padding: '13px 14px', textAlign: 'left', border: `1px solid ${options.fulfillmentMethods?.[key] ? 'var(--accent-primary)' : 'var(--border-color)'}`, borderRadius: '12px', background: options.fulfillmentMethods?.[key] ? 'var(--accent-glow)' : 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 700 }}>{options.fulfillmentMethods?.[key] ? '✓ ' : ''}{label}</button>)}</div></div>
        {options.orderTypes?.order && <div><strong style={{ display: 'block', marginBottom: '9px', fontSize: '13px' }}>Formas de retirada para Encomendas</strong><div style={{ display: 'grid', gap: '9px' }}>{[['delivery', 'Entrega'], ['pickup', 'Retirada na loja'], ['local', 'Consumo no local']].map(([key, label]) => <button type="button" key={`order-${key}`} onClick={() => toggle('orderFulfillmentMethods', key)} style={{ padding: '13px 14px', textAlign: 'left', border: `1px solid ${options.orderFulfillmentMethods?.[key] ? 'var(--accent-primary)' : 'var(--border-color)'}`, borderRadius: '12px', background: options.orderFulfillmentMethods?.[key] ? 'var(--accent-glow)' : 'var(--bg-secondary)', color: 'var(--text-primary)', fontWeight: 700 }}>{options.orderFulfillmentMethods?.[key] ? '✓ ' : ''}{label}</button>)}</div></div>}
      </div>;
    }
    ...
    if (current.type === 'select') {
      const options = current.options || (current.key === 'activeModel' ? ['openai', 'openai-mini', 'openai-gpt5-mini', 'openai-nano', 'claude'] : ['openai', 'claude']);
      const optionLabels = current.optionLabels || (current.key === 'activeModel'
        ? ['OpenAI GPT-4o', 'OpenAI GPT-4o Mini', 'OpenAI GPT-5 Mini', 'OpenAI GPT-4.1 Nano', 'Anthropic Claude']
        : []);
      return <select autoFocus style={inputStyle} value={String(value || options[0] || 'openai')} onChange={(event) => setValue(event.target.value)}>{options.map((option, index) => <option key={option} value={option}>{optionLabels[index] || option}</option>)}</select>;
    }
