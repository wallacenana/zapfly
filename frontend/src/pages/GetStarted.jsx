...
export default function GetStarted() {
  ...
  const [form, setForm] = useState({ openai: '', claude: '', activeModel: 'openai', businessName: '', businessCategory: '', prepTime: '', businessAddress: '', slug: '' });

  useEffect(() => {
    api.get('/config/keys').then(({ data }) => {
      setForm((current) => ({ ...current, openai: data.openai || '', claude: data.claude || '', activeModel: data.activeModel || 'openai', businessName: data.businessName || '', businessCategory: data.businessCategory || '', prepTime: data.prepTime || '', businessAddress: data.businessAddress || '', slug: data.slug || '' }));
