...
const emptyManualForm = (date) => ({ deliveryDate: date, clientName: '', deliveryAddress: '', deliveryFee: '', orderValue: '', paymentMethod: '', paymentReceived: false, deliveryPerson: '', notes: '' });
...
export default function Deliveries() {
  ...
  const saveManualDelivery = async (event) => {
    ...
    setError('');
    try {
      await api.post('/dashboard/deliveries/manual', { ...form, deliveryFee: Number(form.deliveryFee) || 0, orderValue: Number(form.orderValue) || 0 });
      setFormOpen(false);
